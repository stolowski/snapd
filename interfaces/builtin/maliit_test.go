...
func (s *MaliitInterfaceSuite) TestConnectedPlugSnippetUsesSlotLabelAll(c *C) {
	...
	c.Assert(snippet, testutil.Contains, `peer=(label="snap.maliit.*"),`)
}
...
func (s *MaliitInterfaceSuite) TestConnectedPlugSnippetUsesSlotLabelSome(c *C) {
	...
	c.Assert(aasnippets["snap.other.app"], HasLen, 1)
	snippet := string(aasnippets["snap.other.app"][0])
	c.Assert(snippet, testutil.Contains, `peer=(label="snap.maliit.{app1,app2}"),`)
}
...
func (s *MaliitInterfaceSuite) TestConnectedPlugSnippetUsesSlotLabelOne(c *C) {
	...
	c.Assert(aasnippets["snap.other.app"], HasLen, 1)
	snippet := string(aasnippets["snap.other.app"][0])
	c.Assert(snippet, testutil.Contains, `peer=(label="snap.maliit.app"),`)
}
...
func (s *MaliitInterfaceSuite) TestConnectedSlotSnippetUsesPlugLabelAll(c *C) {
	...
	aasnippets := apparmorSpec.Snippets()
	c.Assert(aasnippets, HasLen, 1)
	c.Assert(aasnippets["snap.maliit.maliit"], HasLen, 1)
	snippet := string(aasnippets["snap.maliit.maliit"][0])
	c.Assert(snippet, testutil.Contains, `peer=(label="snap.maliit.*"),`)
}
