...
func (s *MirInterfaceSuite) TestUsedSecuritySystems(c *C) {
	...
	c.Assert(aasnippets["snap.mir-server.mir"], HasLen, 1)
	snippet := string(aasnippets["snap.mir-server.mir"][0])
	c.Assert(snippet, testutil.Contains, "capability sys_tty_config")
	...
	c.Assert(aasnippets["snap.mir-server.mir"], HasLen, 1)
	snippet = string(aasnippets["snap.mir-server.mir"][0])
	c.Assert(snippet, testutil.Contains, "unix (receive, send) type=seqpacket addr=none peer=(label=\"snap.other")
	...
	c.Assert(snippet, testutil.Contains, "/run/mir_socket rw,")
}
