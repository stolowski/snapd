...
func (s *BoolFileInterfaceSuite) TestPlugSnippetDereferencesSymlinks(c *C) {
	...
	c.Assert(snippet, Equals, "(dereferenced)/sys/class/gpio/gpio13/value rwk,\n")
	...
	c.Assert(aasnippets["snap.other.app"], HasLen, 1)
	snippet = string(aasnippets["snap.other.app"][0])
	c.Assert(snippet, Equals, "(dereferenced)/sys/class/leds/input27::capslock/brightness rwk,\n")
}
