...
func (s *Unity8CalendarInterfaceSuite) TestConnectedPlugSnippetUsesSlotLabelAll(c *C) {
	...
	c.Assert(snippet, testutil.Contains, `peer=(label="snap.unity8.*"),`)
}
...
func (s *Unity8CalendarInterfaceSuite) TestConnectedPlugSnippetUsesSlotLabelSome(c *C) {
	...
	c.Assert(aasnippets["snap.other.app"], HasLen, 1)
	snippet := string(aasnippets["snap.other.app"][0])
	c.Assert(snippet, testutil.Contains, `peer=(label="snap.unity8.{app1,app2}"),`)
}
...
func (s *Unity8CalendarInterfaceSuite) TestConnectedPlugSnippetUsesSlotLabelOne(c *C) {
	...
	c.Assert(aasnippets["snap.other.app"], HasLen, 1)
	snippet := string(aasnippets["snap.other.app"][0])
	c.Assert(snippet, testutil.Contains, `peer=(label="snap.unity8.app"),`)
}
...
func (s *Unity8CalendarInterfaceSuite) TestConnectedSlotSnippetAppArmor(c *C) {
	...
	c.Assert(snippets["snap.unity8-calendar.app"], HasLen, 1)
	snippet := string(snippets["snap.unity8-calendar.app"][0])
...
func (s *Unity8CalendarInterfaceSuite) TestPermanentSlotSnippetAppArmor(c *C) {
	...
	c.Assert(snippets["snap.unity8-calendar.app"], HasLen, 1)
	snippet := string(snippets["snap.unity8-calendar.app"][0])
	c.Check(snippet, testutil.Contains, "name=\"org.gnome.evolution.dataserver.Sources5\"")
}
