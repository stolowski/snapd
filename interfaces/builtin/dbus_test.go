...
func (s *DbusInterfaceSuite) TestPermanentSlotAppArmorSession(c *C) {
	...
	c.Assert(aasnippets["snap.test-dbus.test-session-provider"], HasLen, 1)
	snippet := string(aasnippets["snap.test-dbus.test-session-provider"][0])
...
func (s *DbusInterfaceSuite) TestPermanentSlotAppArmorSystem(c *C) {
	...
	aasnippets := apparmorSpec.Snippets()
	c.Assert(aasnippets, HasLen, 1)
	c.Assert(aasnippets["snap.test-dbus.test-system-provider"], HasLen, 1)
	snippet := string(aasnippets["snap.test-dbus.test-system-provider"][0])
...
func (s *DbusInterfaceSuite) TestConnectedPlugAppArmorSession(c *C) {
	...
	aasnippets := apparmorSpec.Snippets()
	c.Assert(aasnippets, HasLen, 4)
	c.Assert(aasnippets["snap.test-dbus.test-session-consumer"], HasLen, 1)
	snippet := string(aasnippets["snap.test-dbus.test-session-consumer"][0])
...
`
	...
	aasnippets := apparmorSpec.Snippets()
	c.Assert(aasnippets, HasLen, 1)
	c.Assert(aasnippets["snap.plugger.app"], HasLen, 1)
	snippet := string(aasnippets["snap.plugger.app"][0])
