...
func (s *FwupdInterfaceSuite) TestConnectedPlugSnippetUsesSlotLabelAll(c *C) {
	...
	c.Assert(len(aasnippets["snap.uefi-fw-tools.app"]), Equals, 1)
	c.Assert(string(aasnippets["snap.uefi-fw-tools.app"][0]), testutil.Contains, `peer=(label="snap.uefi-fw-tools.*"),`)
}
...
func (s *FwupdInterfaceSuite) TestConnectedPlugSnippetUsesSlotLabelSome(c *C) {
	...
	c.Assert(len(aasnippets), Equals, 1)
	c.Assert(len(aasnippets["snap.uefi-fw-tools.app"]), Equals, 1)
	c.Assert(string(aasnippets["snap.uefi-fw-tools.app"][0]), testutil.Contains, `peer=(label="snap.uefi-fw-tools.{app1,app2}"),`)
}
...
func (s *FwupdInterfaceSuite) TestConnectedPlugSnippetUsesSlotLabelOne(c *C) {
	...
	c.Assert(len(aasnippets), Equals, 1)
	c.Assert(len(aasnippets["snap.uefi-fw-tools.app"]), Equals, 1)
	c.Assert(string(aasnippets["snap.uefi-fw-tools.app"][0]), testutil.Contains, `peer=(label="snap.uefi-fw-tools.app2"),`)
}
...
func (s *FwupdInterfaceSuite) TestUsedSecuritySystems(c *C) {
	...
	c.Assert(err, IsNil)
	aasnippets := apparmorSpec.Snippets()
	c.Assert(aasnippets, HasLen, 2)
	c.Assert(aasnippets["snap.uefi-fw-tools.app"], HasLen, 1)
	c.Assert(aasnippets["snap.uefi-fw-tools.app2"], HasLen, 2)
