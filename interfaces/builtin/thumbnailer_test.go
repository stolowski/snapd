...
func (s *ThumbnailerInterfaceSuite) TestUsedSecuritySystems(c *C) {
	...
	c.Assert(aasnippets["snap.thumbnailer.app"], HasLen, 1)
	aasnippet := string(aasnippets["snap.thumbnailer.app"][0])
	c.Assert(aasnippet, testutil.Contains, `interface=com.canonical.Thumbnailer`)
	...
	c.Assert(aasnippets["snap.thumbnailer.app"], HasLen, 1)
	aasnippet = string(aasnippets["snap.thumbnailer.app"][0])
	c.Assert(aasnippet, testutil.Contains, `member={RequestName,ReleaseName,GetConnectionCredentials}`)
}
...
func (s *ThumbnailerInterfaceSuite) TestSlotGrantedAccessToPlugFiles(c *C) {
	...
	c.Assert(aasnippets, HasLen, 1)
	c.Assert(aasnippets["snap.thumbnailer.app"], HasLen, 1)
	snippet := string(aasnippets["snap.thumbnailer.app"][0])
