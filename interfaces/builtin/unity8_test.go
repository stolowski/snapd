...
func (s *unity8InterfaceSuite) TestUsedSecuritySystems(c *C) {
	...
	c.Assert(aasnippets["snap.other.unity8-app"], HasLen, 1)
	aasnippet := string(aasnippets["snap.other.unity8-app"][0])
	c.Check(aasnippet, testutil.Contains, "name=com.canonical.URLDispatcher")
...
func (s *unity8InterfaceSuite) TestSecurityTags(c *C) {
	...
	c.Assert(aasnippets["snap.other.unity8-app"], HasLen, 1)
	aasnippet := string(aasnippets["snap.other.unity8-app"][0])
	c.Check(aasnippet, testutil.Contains, "label=\"snap.unity8-session.*\"")
}
